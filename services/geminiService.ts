...
export class GeminiService {
  ...
  async generateMetadata(
    ...
    if (filename) {
      ...
      // Sort by length desc to ensure "Veo 3" matches before "Veo"
      const markers = [
      ...
      if (clean.length > 3) {
        titleContext = `CONTEXT FROM FILENAME: "${clean}"
(Use this title context to guide tags and description. If image analysis is unclear or unavailable, rely on this title.)`;
      }
    }
    ...
    const promptParts: any[] = [
      { text: `Analyze the following content and generate SEO metadata. 
${titleContext}
