...
export class GeminiService {
  ...
  async generateMetadata(
    ...
    // Clean filename to use as context
    let titleContext = "";
    if (filename) {
      // Remove extension
      let clean = filename.replace(/\.[^/.]+$/, "");
      
      // Remove AI markers (case insensitive)
      const markers = [
      ...
      markers.forEach(marker => {
          // Escape special regex chars if any (none in this list really, but good practice)
          const regex = new RegExp(marker, "gi");
          clean = clean.replace(regex, "");
      });
      
      // Clean up extra spaces, underscores, hyphens
      clean = clean.replace(/[_\-]/g, " ").replace(/\s+/g, " ").trim();
      
      if (clean.length > 3) {
        titleContext = `Context from filename: "${clean}". Use this to guide the title and keywords, but ensure the final output accurately describes the visual content.`;
      }
    }
    ...
    const promptParts: any[] = [
      { text: `Analyze this image/video and generate SEO metadata. 
${titleContext}
Profile: ${profile}. ${profileInstruction} ${variantModifier}
